...
class SpectralNormConv(object):
    ...
    def compute_weight(self, module, do_power_iteration):
        ...
        output_padding = 0 if stride[0]==1 else 1

        if groups != 1:
          module.weight_u = normalize(torch.randn(module.weight_u.size()).to(module.weight_u.device), dim=0, out=module.weight_u)
          module.weight_v = normalize(torch.randn(module.weight_v.size()).to(module.weight_v.device), dim=0, out=module.weight_v)
        ...
        """
        if groups != 1:
          self.n_power_iterations = 10
          u = torch.randn(u.size())
          v = torch.randn(v.size())
            ...
            with torch.no_grad():
                for _ in range(self.n_power_iterations):
                    v_s = conv_transpose2d(u.view(self.out_shape), weight, stride=stride,
                                         padding=padding, groups=groups, dilation=dilation,output_padding=output_padding)
                    # Note: out flag for in-place changes
                    v = normalize(v_s.view(-1), dim=0, eps=self.eps, out=v)

                    u_s = conv2d(v.view(self.input_dim), weight, stride=stride, padding=padding, groups=groups,
                ...
                if self.n_power_iterations > 0:
                    ...
                    u = u.clone()
                    v = v.clone()
        weight_v = conv2d(v.view(self.input_dim), weight, stride=stride, padding=padding, groups=groups, dilation=dilation,
                           bias=None)
        ...
        ####### rescale spectral norm around 1
        factorReverse = sigma / self.coeff
        ####### rescale spectral norm around 1
        # rescaling
    ...
    def apply(module, coeff, input_dim, name, n_power_iterations, eps):
        ...
        with torch.no_grad():
            ...
            u = conv2d(v.view(input_dim), weight, stride=stride, padding=padding, groups=groups, dilation=dilation,
                               bias=None)
